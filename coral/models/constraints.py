...
def get_eulerian_node_constraints(model: pyo.ConcreteModel, k: int, bp_graph: BreakpointGraph) -> List[pyo.Constraint]:
        ...
        if node in endnode_list:
            ...
            for i in range(k):
                ...
                for di in bp_graph.nodes[node][2]:
                    ec_expr -= model.x[(bp_graph.num_seq_edges + bp_graph.num_conc_edges + di), i]
                for srci in bp_graph.nodes[node][3]:
                    ec_expr -= model.x[(bp_graph.num_nonsrc_edges + 2 * srci), i]  # connected to s
                    ec_expr -= model.x[(bp_graph.num_nonsrc_edges + 2 * srci) + 1, +i]  # connected to t
...
def get_spanning_tree_constraints(
    ...
    for i in range(k):
        ...
        for node in endnode_list:
            ...
            t_expr_yd += model.y1[idx_offset + 1, i] * (model.dt[i] - model.d[node_order[node], i])  # node -> t
            expr_x = 0.0  # linear
            expr_y = 0.0  # linear
            expr_xc = 0.0  # quad
            expr_yd = 0.0  # quad
...
def set_copy_number_constraints(
    ...
    model.ConstraintCNSequence = pyo.Constraint(
        model.seq_edge_idx,
        rule=lambda model, seqi: sum(model.w[i] * model.x[seqi, i] for i in range(k)) <= edge_to_cn.sequence[seqi],
    )
...
def get_cycle_weight_constraint(model: pyo.Model, bp_graph: BreakpointGraph) -> pyo.Constraint:
