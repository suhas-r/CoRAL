...
def get_minimize_objective(
    model: pyo.Model, bp_graph: BreakpointGraph, k: int, total_weights: float, pc_list: List, is_post: bool
) -> pyo.Objective:
...
class CycleLPModel(pyo.ConcreteModel):
    ...
    def __init__(self, name: str, bp_graph: BreakpointGraph, k: int, pc_list: List, is_post: bool = False) -> None:
        ...
        self.w = pyo.Var(self.k, domain=pyo.NonNegativeReals, bounds=(0.0, bp_graph.max_cn))
        # x: the number of times an edge occur in cycle or path i
        self.x = pyo.Var(self.edge_idx, self.k, domain=pyo.Integers, bounds=(0.0, 10.0))
        ...
        self.d = pyo.Var(self.node_idx, self.k, within=pyo.NonNegativeIntegers, bounds=(0.0, bp_graph.num_nodes + 2))
        self.ds = pyo.Var(self.k, within=pyo.NonNegativeIntegers, bounds=(0.0, bp_graph.num_nodes + 2))
        self.dt = pyo.Var(self.k, within=pyo.NonNegativeIntegers, bounds=(0.0, bp_graph.num_nodes + 2))

        # y: spanning tree indicator (directed)
        self.y1 = pyo.Var(self.edge_idx, self.k, domain=pyo.Binary)  # Positive direction
        self.y2 = pyo.Var(self.edge_idx, self.k, domain=pyo.Binary)  # Negative direction
        # endregion
...
def get_model(
    ...
    model = CycleLPModel(name=f"{model_name}_{k}", bp_graph=bp_graph, k=k, pc_list=pc_list, is_post=is_post)
    ...
    if not is_greedy:
        ...
        model.ObjectiveGreedy = get_greedy_objective(model, bp_graph, pc_list, unsatisfied_pc or [], pp)
    ...
    for constraint in constraints.get_eulerian_node_constraints(model, k, bp_graph):
        model.ConstraintEulerianNodes.add(constraint)

    constraints.set_copy_number_constraints(model, k, bp_graph, remaining_cn or EdgeToCN.from_graph(bp_graph), p_bp_cn)

    model.ConstraintBPOccurrence = constraints.get_bp_occurrence_constraint(model, bp_graph)
    model.ConstraintCycleWeight = constraints.get_cycle_weight_constraint(model, bp_graph)
    ...
    for i in range(k):
        for di in range(bp_graph.num_disc_edges):
            ...
            if dedge[0] == dedge[3] and dedge[1] == dedge[4] and dedge[2] == dedge[5]:  # exclude self loops
                model.ConstraintXY1D.add(model.y1[(bp_graph.num_seq_edges + bp_graph.num_conc_edges + di), i] == 0)
                model.ConstraintXY2D.add(model.y2[(bp_graph.num_seq_edges + bp_graph.num_conc_edges + di), i] == 0)

    model.ConstraintsXYD = pyo.ConstraintList()
    for constraint in constraints.get_spanning_tree_constraints(model, k, bp_graph, node_order):
        model.ConstraintsXYD.add(constraint)
    for constraint in constraints.get_spanning_tree_constraints__non_endnode(model, k, bp_graph, node_order):
        model.ConstraintsXYD.add(constraint)
