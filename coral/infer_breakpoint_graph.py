        ...
        for amplicon_idx in range(len(self.lr_graph)):
            for seqi in range(len(self.lr_graph[amplicon_idx].sequence_edges)):
                ...
                if seg[5] == -1:
                    ...
                    logger.debug(f"LR cov assigned for sequence edge {seg[:3]}.")
        """
            ...
            for di in range(ld):
                ...
                for r_ in bp[10]:
                    if r_[1] == r_[2]:
                        ...
                        bp_reads[r_[0]] = [[[r_[1], r_[2], di]], []]
            logger.debug(f"There are {len(bp_reads)} reads covering >=1 breakpoint in amplicon {amplicon_idx + 1}.")
            for rn in bp_reads:
                ...
                for pi in range(len(paths)):
                    path = paths[pi]
                    if len(path) > 5 and path_constraints.valid_path(self.lr_graph[amplicon_idx], path):
...
def reconstruct_graph(
    lr_bam_filename: str,
    cnv_seed_file: click.File,
    cn_seg_file: click.File,
    output_prefix: str,
    output_bp: bool,
