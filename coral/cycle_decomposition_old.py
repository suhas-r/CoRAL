...
def minimize_cycles(
    ...
    z = m.addVars(k, vtype=GRB.BINARY, name=["z" + str(i) for i in range(k)])  # type: ignore[call-overload]

    # w[i]: the weight of cycle or path i, continuous variable
    w = m.addVars(k, lb=0.0, ub=bp_graph.max_cn, vtype=GRB.CONTINUOUS, name=["w" + str(i) for i in range(k)])  # type: ignore[call-overload]

    # Relationship between w[i] and z[i]
    for i in range(k):
        m.addConstr(w[i] <= z[i] * bp_graph.max_cn)
    ...
    x = m.addVars(k * nedges, lb=0.0, ub=10.0, vtype=GRB.INTEGER, name=x_names)  # type: ignore[call-overload]
    ...
    for i in range(k):
        ...
        for seqi in range(lseg):
            obj -= x[seqi * k + i] * w[i] * bp_graph.sequence_edges[seqi][-2] / total_weights
    ...
    for seqi in range(lseg):
        ...
        m.addQConstr(cn_expr <= bp_graph.sequence_edges[seqi][-1], name="seq_cn")  # type: ignore[operator]
    for ci in range(lc):
        cn_expr = gp.QuadExpr(0.0)
        for i in range(k):
            cn_expr += w[i] * x[(lseg + ci) * k + i]
        m.addQConstr(cn_expr <= bp_graph.concordant_edges[ci][-1], name="conc_cn")
    for di in range(ld):
        cn_expr = gp.QuadExpr(0.0)
        for i in range(k):
            cn_expr += w[i] * x[(lseg + lc + di) * k + i]
        m.addQConstr(cn_expr <= bp_graph.discordant_edges[di][-1], name="disc_cn1")
        m.addQConstr(cn_expr >= p_bp_cn * bp_graph.discordant_edges[di][-1], name="disc_cn2")
    for srci in range(lsrc):
        cn_expr = gp.QuadExpr(0.0)
        for i in range(k):
            cn_expr += w[i] * x[(lseg + lc + ld + 2 * srci) * k + i]
            cn_expr += w[i] * x[(lseg + lc + ld + 2 * srci) * k + k + i]
        m.addQConstr(cn_expr <= bp_graph.source_edges[srci][-1], "src_cn")

    # Occurrence of breakpoints in each cycle/path
    discordant_multiplicities = bp_graph.infer_discordant_edge_multiplicities()
    for i in range(k):
        for di in range(ld):
            m.addConstr(x[(lseg + lc + di) * k + i] <= discordant_multiplicities[di], name=f"bp_occ_{i}_{di}")
    ...
    c = m.addVars(k * nnodes, vtype=GRB.BINARY, name=c_names)  # type: ignore[call-overload]
    ...
    for i in range(k):
        ...
        m.addConstr(cycle_expr <= 1.0, name="constr_cx")
    ...
    for i in range(k):
        ...
        m.addConstr(expr_xc <= 1.0, name="constr_single_bp")
    ...
    # including s, t, at the end
    d = m.addVars(k * (nnodes + 2), lb=0.0, ub=nnodes + 2, vtype=GRB.INTEGER, name=d_names)  # type: ignore[call-overload]
    ...
    y1 = m.addVars(k * nedges, vtype=GRB.BINARY, name=y1_names)  # type: ignore[call-overload] # small -> large
    y2 = m.addVars(k * nedges, vtype=GRB.BINARY, name=y2_names)  # type: ignore[call-overload] # large -> small

    # Relationship between c and d
    for i in range(k):
        for ni in range(nnodes):
            m.addConstr(d[ni * k + i] >= c[ni * k + i], name="constr_cd1")
    for i in range(k):
        d_expr = gp.LinExpr(0.0)
        for ni in range(nnodes):
            d_expr += c[ni * k + i]
        d_expr += d[k * nnodes + i]  # d_s,i
        m.addConstr(d_expr <= 1.0, name="constr_cd2")

    # Relationship between y and z:
    for i in range(k):
        for j in range(nedges):
            m.addConstr(y1[j * k + i] <= z[i], name="constr_y1z")
            m.addConstr(y2[j * k + i] <= z[i], name="constr_y2z")

    # Relationship between x, y and d
    for i in range(k * nedges):
        m.addConstr(y1[i] + y2[i] <= x[i], name="constr_xy")
    for i in range(k):
        for di in range(ld):
            dedge = bp_graph.discordant_edges[di]
            if dedge[0] == dedge[3] and dedge[1] == dedge[4] and dedge[2] == dedge[5]:  # exclude self loops
                m.addConstr(y1[(lseg + lc + di) * k + i] == 0, name="constr_xy1d")
                m.addConstr(y2[(lseg + lc + di) * k + i] == 0, name="constr_xy2d")
    ctr = [1] * 4
    for i in range(k):
        ...
        for node in endnode_list:
            ...
            t_expr_yd += y1[(lseg + lc + ld + 2 * lsrc + 2 * endnode_list.index(node)) * k + k + i] * (
                d[k * nnodes + k + i] - d[k * node_order[node] + i]
            )  # node -> t
            ...
            expr_xc += (
                x[(lseg + lc + ld + 2 * lsrc + 2 * endnode_list.index(node)) * k + i] * c[k * node_order[node] + i]
            )
            expr_x += x[(lseg + lc + ld + 2 * lsrc + 2 * endnode_list.index(node)) * k + k + i]  # to t
            expr_xc += (
                x[(lseg + lc + ld + 2 * lsrc + 2 * endnode_list.index(node)) * k + k + i] * c[k * node_order[node] + i]
            )
            ...
            m.addConstr(expr_x * (nnodes + 2) >= d[k * node_order[node] + i], name=f"constr_xyd_{ctr[0]}")

            ctr[0] += 1
            m.addConstr(expr_y <= 1.0, name=f"constr_xyd_{ctr[0]}")
            ctr[0] += 1
            m.addConstr(expr_y * nedges * k + expr_xc >= expr_x, name="constr_xyd2")
            m.addConstr(expr_yd * nedges * k + expr_xc >= expr_x, name="constr_xyd2")
        ...
        m.addConstr(t_expr_x * (nnodes + 2) >= d[k * nnodes + k + i], name="constr_xyd3")
        m.addConstr(t_expr_y <= 1.0, name="constr_xyd3")
        m.addConstr(t_expr_y * nedges * k >= t_expr_x, name="constr_xyd4")
        m.addConstr(t_expr_yd >= t_expr_x, name="constr_xyd4")
            ...
            if node not in endnode_list:
                ...
                for di in bp_graph.nodes[node][2]:
                    ...
                    if node_order[node_] <= node_order[node]:
                        ...
                        expr_yd += y1[(lseg + lc + di) * k + i] * (
                            d[k * node_order[node] + i] - d[k * node_order[node_] + i]
                        )
                    else:
                        expr_y += y2[(lseg + lc + di) * k + i]
                        expr_yd += y2[(lseg + lc + di) * k + i] * (
                            d[k * node_order[node] + i] - d[k * node_order[node_] + i]
                        )
                for srci in bp_graph.nodes[node][3]:
    ...
    r = m.addVars(k * len(pc_list), vtype=GRB.BINARY, name=r_names)  # type: ignore[call-overload]
    for pi in range(len(pc_list)):
        ...
        for edge in path_constraint_.keys():
            for i in range(k):
                if edge[0] == "s":
                    ...
                    m.addConstr(
                        x[(lseg + edge[1]) * k + i] >= r[pi * k + i] * path_constraint_[edge],
                    )
                else:
                    m.addConstr(
                        x[(lseg + lc + edge[1]) * k + i] >= r[pi * k + i] * path_constraint_[edge],
                    )
...
def minimize_cycles_post(
    ...
    # Gurobi model
    m = gp.Model(
        model_prefix + "_amplicon" + str(amplicon_id) + "_cycle_postprocessing_k=" + str(k),
    )
    ...
    # w[i]: the weight of cycle or path i, continuous variable
    w = m.addVars(  # type: ignore[call-overload]
        k,
        lb=0.0,
        ub=g.max_cn,
    ...
    if len(pc_list) > 0:
        ...
        for pi in range(len(pc_list)):
            ...
            for i in range(k):
                r_names.append("r" + str(pi) + "," + str(i))
        r = m.addVars(k * len(pc_list), vtype=GRB.BINARY, name=r_names)  # type: ignore[call-overload]
        R = m.addVars(len(pc_list), vtype=GRB.BINARY, name=R_names)  # type: ignore[call-overload]
    ...
    for seqi in range(lseg):
        ...
        for i in range(k):
            cn_expr += w[i] * x[seqi * k + i]
        m.addQConstr(cn_expr <= g.sequence_edges[seqi][-1])  # type: ignore[operator]
    for ci in range(lc):
        cn_expr = gp.QuadExpr(0.0)
        for i in range(k):
            cn_expr += w[i] * x[(lseg + ci) * k + i]
    ...
    for i in range(k):
        d_names.append("dt," + str(i))
    d = m.addVars(  # type: ignore[call-overload]
        k * (nnodes + 2),
        lb=0.0,
        ub=nnodes + 2,
        vtype=GRB.INTEGER,
...
def maximize_weights_greedy(
    ...
    while next_w >= resolution and (
        ...
        for node in g.nodes.keys():
            if node in g.endnodes:
                m.addConstr(
                    x[(lseg + lc + ld + 2 * lsrc + 2 * endnode_list.index(node))]
        ...
        for node in endnode_list:
            ...
            expr_yd += y1[lseg + lc + ld + 2 * lsrc + 2 * endnode_list.index(node)] * (d[node_order[node]] - d[nnodes])
